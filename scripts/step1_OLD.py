...
# 改良方針:
# 1. ASIN / JAN は完全一致のみ
# 2. 禁止ブランドは brand列 + title のみで判定し、amazon_url / raw_text全文では判定しない
# 3. 禁止ワードは title + brand + 安全な元CSV文字列で判定する
# 4. Amazon URL、画像URL、レビュー/広告/内部文言っぽい長文はNG判定対象から除外する
# 5. 短い英数字NGは contains ではなく単語一致にする
# 6. category_rules.csv の allow / deny は title のみで判定する
# 7. category_rules.csv の match_mode は contains / exact / word に対応
#
# contains: 文言の中に含まれていれば一致
# exact   : 正規化後の文字列全体が完全一致した場合だけ一致
# word    : 単語として一致した場合だけ一致。短い英数字向け
# =========================================================
...
def is_short_english_or_code(value):
    """
    JOY / AVG / DVD / HP / CA / 3M など、短くて誤爆しやすい英数字判定。
    """
    raw = str(value).strip()
...
def match_value(target_text, rule_value, match_mode="contains"):
    ...
    contains:
      正規化後、対象文字列のどこかに rule_value が含まれれば一致。
      例: 冷蔵 は 冷蔵庫 に一致する。

    exact:
      正規化後、対象文字列全体と rule_value が完全一致した時だけ一致。
      例: title全体が「dvd」だけなら一致。長い商品名内のDVDには一致しない。

    word:
      空白・記号で分割した単語として一致。
      例: Joy は Enjoy には一致しないが、Joy 単体には一致する。
    """
    if not rule_value:
        return False
...
def safe_match_mode_for_ng(value):
    """
    ng_master.csvにはmatch_mode列が無いため、自動で安全側に寄せる。
    短い英数字だけ word、それ以外は contains。
    """
...
def is_noise_text(text):
    """
    Amazon内部文言・レビュー・広告など、NG判定に混ぜると誤爆しやすい文字列を除外。
    """
...
def decode_amazon_product_slug(url):
    """
    URL全文ではなく、/商品名/dp/ASIN の商品名スラッグだけを補助的に取り出す。
    JOY/AVGのようなURLランダム文字列誤爆を避ける。
    """
    if not url:
        return ""
...
def build_raw_text_for_ng(row):
    """
    NG判定補助用テキスト。
    画像URL、Amazon URL全文、長すぎる内部文言は入れない。
    Amazon URLからは商品名スラッグだけを抽出する。
    """
    parts = []
...
def find_amazon_csv_files():
    ...
    for file in INPUT_DIR.glob("*.csv"):
        if file.name.lower() in ["ng_master.csv", "category_rules.csv"]:
            continue
...
def load_ng_master():
    ...
    想定ヘッダー:
    ASIN,JAN,キーワード,商品説明,ブランド,カテゴリ

    重要:
    - ASIN / JAN は完全一致
    - ブランドは brand/title のみで判定
    - キーワード/商品説明/カテゴリは title/brand/raw_text_for_ng で判定
    - 短い英数字は自動で word 判定
    """
    empty = {
        "asin_set": set(),
...
def judge_ng(row, ng_data):
    ...
    重要:
    brand NGをamazon_urlやraw_text全文には掛けない。
    これで JOY / AVG / DVD のようなURL・内部文言誤爆を抑える。
    """
    ...
    if asin and asin in ng_data["asin_set"]:
        return "OUT", "asin", asin, "NG ASINに完全一致"

    if jan and jan in ng_data["jan_set"]:
        return "OUT", "jan", jan, "NG JANに完全一致"

    brand_target = " ".join([brand, title])
    title_brand_target = " ".join([title, brand])
    ...
    for ng in ng_data["brand_ng"]:
        if match_value(brand_target, ng["ng_value"], ng["match_mode"]):
            return "OUT", "brand", ng["ng_value"], f"NGブランド一致({ng['match_mode']})"

    for ng in ng_data["keyword_ng"]:
        if match_value(product_text_target, ng["ng_value"], ng["match_mode"]):
            return "OUT", "keyword", ng["ng_value"], f"NGキーワード一致({ng['match_mode']})"

    for ng in ng_data["description_ng"]:
        if match_value(product_text_target, ng["ng_value"], ng["match_mode"]):
            return "OUT", "description", ng["ng_value"], f"NG商品説明一致({ng['match_mode']})"

    for ng in ng_data["category_ng"]:
        if match_value(category_target, ng["ng_value"], ng["match_mode"]):
            return "OUT", "category", ng["ng_value"], f"NGカテゴリ一致({ng['match_mode']})"
...
def judge_category(row, category_name, rule_df):
    """
    category_rules.csv 判定。
    categoryルールはtitleのみで判定する。

    判定順:
    1. denyに当たればOUT
    2. allowに当たればSAFE
    3. allowに当たらなければCATEGORY_CHECK
    4. ルール未設定ならCATEGORY_CHECK
    """
    title = str(row.get("title", "")).strip()
    ...
    for _, rule in deny_rules.iterrows():
        ...
        if match_value(title, value, match_mode):
            return "OUT", "category_deny", value, f"カテゴリ除外ワード一致({match_mode})"
...
def process_one_amazon_csv(amazon_csv, ng_data, category_rules_df, timestamp, logs):
    process_start = time.time()
    category_name = get_category_from_filename(amazon_csv)
    ...
    for _, row in standard_df.iterrows():
        ...
        elif cat_status == "CATEGORY_CHECK":
            row_dict["status"] = "CATEGORY_CHECK"
            row_dict["hit_type"] = cat_hit_type
            row_dict["hit_value"] = cat_hit_value
            row_dict["out_reason"] = cat_reason
        else:
            row_dict["status"] = "SAFE"
            row_dict["hit_type"] = cat_hit_type
            row_dict["hit_value"] = cat_hit_value
            row_dict["out_reason"] = cat_reason
    ...
    out_df = judged_df[judged_df["status"] == "OUT"].copy()
    ...
    out_path = OUT_DIR / f"amazon_out_{category_name}_{timestamp}.csv"
