...
# 8. Shopee出品済みASINは OUT_DUPLICATE として別出力する
# 9. OUTは OUT_NG / OUT_DENY / OUT_DUPLICATE に分離する
#
# 判定順:
# 1. NG判定 → OUT_NG
# 2. Shopee出品済みASIN判定 → OUT_DUPLICATE
# 3. category_rules deny → OUT_DENY
# 4. category_rules allow → SAFE
# 5. allow未該当・ルール未設定 → CATEGORY_CHECK
# =========================================================
...
def load_existing_asins():
    ...
    想定ファイル:
    input/shopee_existing_asin.csv

    想定ヘッダー:
    asin,source_file,sheet,source_row,sku

    重要:
    - 判定には asin 列だけを使用
    - 既出品はNGではなく OUT_DUPLICATE として分離
    """
...
def judge_duplicate(row, existing_asin_set):
    """
    Shopee出品済みASIN判定。
    既出品はNGではなく OUT_DUPLICATE として分離する。
    """
...
def judge_category(row, category_name, rule_df):
    ...
    判定順:
    1. denyに当たれば OUT_DENY
    2. allowに当たれば SAFE
    3. allowに当たらなければ CATEGORY_CHECK
    4. ルール未設定なら CATEGORY_CHECK
    """
    title = str(row.get("title", "")).strip()
